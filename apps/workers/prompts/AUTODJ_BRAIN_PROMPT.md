...
- Score < 50: FILTER_SWEEP ou ECHO_OUT préférés (Hard Cut en dernier recours)
...
### HARD_CUT (Dernier recours)

Conditions: Changement radical de BPM (> 10%) ou de genre IMPOSSIBLE à mixer
Durée: 0 bars (instantané)
Options: sec, reverb_tail, delay_tail
**Note**: Éviter si possible. Préferer ECHO_OUT ou FILTER_SWEEP pour les clashs.
...
| PEAK      | 8-16 bars        | Varié, ECHO_OUT préféré à HARD_CUT |
...
```json
{
  ...
  "warnings": [{ "type": "INFO", "message": "Transition optimale, aucun problème détecté" }],
